...
def PD(on_pseudolayer, keycodes_hash, keycode, output_buffer, index):
    return new_chord(on_pseudolayer, keycodes_hash, False, keycode, 0, "single_dance_mouse", output_buffer, index)
    ...
    if DEFINITION == "":
        ...
        if len(split) == 1:
            ...
            if type == "O":
                ...
                val1 = val.split(',')[0].strip()
                [output_buffer, index] = PD(PSEUDOLAYER, KEYCODES_HASH, val1, output_buffer, index)
            elif type == "MO":
                if not ',' in val:
